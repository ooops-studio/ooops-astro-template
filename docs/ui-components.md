...
Run `pnpm generate:editor` after changing design tokens. `pnpm check:editor` rejects stale CSS, missing component sources, unresolved package manifests, unsafe token references and writable Stage bindings. Components may expose `editorId`; markers are emitted only in development with `OOOPS_EDITOR_MODE=1`.
